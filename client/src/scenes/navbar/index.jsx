...
const Navbar = () => {
  ...
  const dispatch = useDispatch(); //to dispatch actions from the reducers (redux)
  const navigate = useNavigate();
  const user = useSelector((state) => state.user);
  const isNonMobileScreens = useMediaQuery("(min-width: 1000px)"); //a hook in mui that allows us to determine if the current screen size is less or more than the min width ; easy way to use mediaquery
